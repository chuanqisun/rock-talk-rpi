...
function resolvePythonRuntime(explicitPythonCommand) {
  ...
  const pythonCommand = existsSync(virtualEnvPython3) ? virtualEnvPython3 : existsSync(virtualEnvPython) ? virtualEnvPython : "python3";
...
export default class LedController {
  ...
  async startPlaybackProgress({ durationMs, introDelayMs = 1000 }) {
    if (this.#disabled) {
      return;
    }
    ...
    this.#send({ type: "playbackProgress", durationMs, introDelayMs });
    ...
    this.#readyPromise = new Promise((resolvePromise) => {
      ...
      stdoutReader.on("line", (line) => {
        ...
        if (message.type === "ack") {
          ...
          if (message.command === "playbackProgress") {
            console.log(`[led] playback progress -> ${message.durationMs}ms (delay ${message.introDelayMs}ms).`);
            return;
          }

          console.log(`[led] ack -> ${message.command}.`);
          return;
        }
